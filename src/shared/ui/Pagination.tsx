...
/**
 * LoadMoreButton — the single standardized "Load More" trigger (infinite
 * list pattern) for all non-POS routes.
 */
...
export function LoadMoreButton({
  ...
  return (
    <button
      ...
      disabled={loading}
      className={cn(
        'btn btn-secondary btn-sm !px-6 !py-2.5 w-full sm:w-auto mx-auto',
        className
      )}
    >
      {loading ? (
      ...
      {label}
      <span className="text-gray-400 font-bold">{visibleCount}/{totalCount}</span>
    </button>
  );
}
