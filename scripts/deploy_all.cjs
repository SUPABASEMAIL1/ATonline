...
async function main() {
  for (const filename of envFiles) {
    ...
    try {
      const payload = JSON.stringify({ query: masterSchema });
      const res = execSync(`curl -s -X POST "https://api.supabase.com/v1/projects/${supabaseRef}/database/query" \\
        -H "Authorization: Bearer ${mgmtKey}" \\
        -H "Content-Type: application/json" \\
        -d @-`, { input: payload }).toString();
      
      const resJson = JSON.parse(res);
      if (resJson.error) {
        console.error('❌ Schema Error:', resJson.error);
      } else {
